...
export function SendDesktop({ isTheme, qrCode, openFileSelector }: SendUIProps) {
        ...
        <div>
            <div className={`min-h-screen flex flex-col  ${isTheme ? 'bg-dark-background' : 'bg-light-background'}`}>
                ...
                <nav className="flex items-center  h-[20vh]">
                    <svg xmlns="http://www.w3.org/2000/svg" height="35px" viewBox="0 -960 960 960" width="35px" fill={isTheme ? '#FFFFFF' : '#000000'} className={`ml-10 rounded-full ${isTheme ? 'hover:bg-zinc-700' : 'hover:bg-neutral-200'}  clickable`}
    ...
    return (
        ...
        `}>
            <nav className="flex justify-between p-6">
                <img src={isTheme ? darkBack : lightBack} alt="back" className="h-[40px]" onClick={() => navigate('/', { replace: true })} />
                <ToggleThemeButton />
            </nav>
            ...
            {qrCode ? (
                ...
                (<div className="flex flex-col items-center">
                    ...
                    <div
                        className={`h-[30vh] w-[54vw] flex items-center justify-center || ${isTheme ? `bg-[#3C3C3C]` : `bg-[#D6FEFF]`} ${isTheme ? `border-[#919191]` : `border-[#A1F3FF]`} border-4 rounded-[20px] mt-[4vh]`} onClick={openFileSelector}
                    >
                        <div className={`h-[25vh] w-[45vw] ${isTheme ? `bg-[#A1A1A1]` : `bg-[#A1F3FF]`} || flex items-center justify-center rounded-[12px]`}>

                            <img src={isTheme ? darksend : lightsend} alt="" className="w-[37vw]" />
                        </div>
                    </div>
...
export default function SendUI({ isTheme, qrCode, openFileSelector }: SendUIProps) {

    const isMobile = useContext(DeviceProvider)?.isMobile

    return (isMobile ? <SendMobile isTheme={isTheme} qrCode={qrCode} openFileSelector={openFileSelector} />
        :
        <SendDesktop isTheme={isTheme} qrCode={qrCode} openFileSelector={openFileSelector} />
    );
}
